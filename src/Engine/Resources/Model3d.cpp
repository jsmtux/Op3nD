...
Model3d::Model3d(string name, Assimp::Importer *_importer, GLuint *_buffers, vector<Resource*> _maps, vector<MeshEntry> _meshes, map<string,unsigned int> _boneMapping, 
	    unsigned int _numBones, vector<BoneInfo> _boneInfo, Matrix _globalInverseTransform, const aiScene* _scene,
	    float _animTime, Matrix _scaleTransform): Resource(name), importer(_importer){
    currentAnimation = 0;
    m_Buffers = _buffers;
    maps = _maps;
    meshes = _meshes;
